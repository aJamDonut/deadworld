 ...
 * Compiled Wed, 13 Dec 2023 00:19:27 UTC
 *
...
(function (exports, core, constants, sprite, math, ticker$1, display) {
	...
	function getBlendMode(name) {
		if (!name) return constants.BLEND_MODES.NORMAL;
	...
	class Emitter {
		...
		init(config) {
			...
			this.spawnChance =
				typeof config.spawnChance === "number" && config.spawnChance > 0 ? config.spawnChance : 1;
		...
		update(delta) {
			...
			if (this._emit) {
				...
				while (this._spawnTimer <= 0) {
					...
					for (
						let len = Math.min(this.particlesPerWave, this.maxParticles - this.particleCount), i = 0;
						i < len;
						++i
					) {
	...
	function upgradeConfig(config, art) {
		...
		if (config.acceleration && (config.acceleration.x || config.acceleration.y)) {
			...
			if ("start" in config.speed) {
				minStart =
					config.speed.start *
					((_a = config.speed.minimumSpeedMultiplier) !== null && _a !== void 0 ? _a : 1);
				maxStart = config.speed.start;
			} else {
				minStart =
					config.speed.list[0].value *
					((_b = config.minimumSpeedMultiplier) !== null && _b !== void 0 ? _b : 1);
				...
				if ("start" in config.speed) {
					if (config.speed.start === config.speed.end) {
						out.behaviors.push({
							...
							config: {
								min:
									config.speed.start *
									((_f = config.speed.minimumSpeedMultiplier) !== null && _f !== void 0 ? _f : 1),
					...
					out.behaviors.push({
						...
						config: {
							min:
								config.speed.list[0].value *
								((_g = config.minimumSpeedMultiplier) !== null && _g !== void 0 ? _g : 1),
		...
		if (
			...
			((_m = config.rotationSpeed) === null || _m === void 0 ? void 0 : _m.min) ||
			((_o = config.rotationSpeed) === null || _o === void 0 ? void 0 : _o.max)
		) {
			out.behaviors.push({
				type: "rotation",
				config: {
			...
			((_t = config.startRotation) === null || _t === void 0 ? void 0 : _t.min) ||
			((_u = config.startRotation) === null || _u === void 0 ? void 0 : _u.max)
		) {
			out.behaviors.push({
				type: "rotationStatic",
				config: {
	...
	Emitter.upgradeConfig = upgradeConfig;
