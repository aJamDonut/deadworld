(function () {
  ...
  _BLUEPRINTS.COMPONENTS.hud_researchmenu = {
    ...
    "getElement": function (options) {
      ...
      const knownButton = game.render.component('hud_button', {
        name: '<< Researched >>'
      });
      knownButton.x = 0;
      knownButton.y = -40;
      stage.addChild(knownButton);
      let knownRecipesVisible = false;
      let knownRecipesComponent = null;
      knownButton.on('pointerup', () => {
        if (knownRecipesVisible) {
          if (knownRecipesComponent) {
            knownRecipesComponent.destroy();
            knownRecipesComponent = null;
          }
          knownRecipesVisible = false;
        } else {
          knownRecipesComponent = game.render.component('hud_unlocked_recipes');
          knownRecipesComponent.x = 0;
          knownRecipesComponent.y = knownButton.y + knownButton.height;
          stage.addChild(knownRecipesComponent);
          knownRecipesVisible = true;
        }
      });
      return stage;
    },
  ...
  _BLUEPRINTS.COMPONENTS.hud_unlocked_recipes = {
    "getElement": function (options) {
      if (!options) {
        options = {
          known: game.session.data.unlocked_recipes
        };
      }
      const categories = Object.keys(options.known);
      categories.splice(0, 1);
      const tabNames = categories.map(category => category.charAt(0).toUpperCase() + category.slice(1));
      const tabComponents = categories.map(category => {
        const items = Object.keys(options.known[category]).map(recipeName => {
          const recipe = game.session.getRecipe(recipeName);
          const itemIcon = game.render.component('component_item_icon', {
            icon: recipe.sprite,
            w: 64,
            h: 64
          });
          itemIcon.x = 32 + 32;
          const itemName = game.render.text(recipe.name, 'ingame-label');
        ...
        const scrollbox = game.render.component('component_scrollbox', {
          w: 600,
          h: 560
        });
        items.forEach((item, index) => {
          item.x = index % 4 * 120;
          item.y = Math.floor(index / 4) * 100;
          scrollbox.addChild(item);
        });
        return scrollbox;
      });
      const hud = game.render.component('component_container_tabs', {
        tabNames: tabNames,
        tabComponents: tabComponents,
        w: 600,
        h: 600
    ...
    "parent": 'component_huds',
    "name": 'HUD Unlocked Recipes'
  };
  _BLUEPRINTS.COMPONENTS.hud_sprite_picker = {
    "getElement": function (options) {
      if (!options) {
        options = {};
      }
      const pane = new UIContainer();
      options.itemsPerPage = options.itemsPerPage || 3 * 3;
      options.spriteSize = options.spriteSize || 100;
      options.sprites = options.sprites || Object.keys(_BLUEPRINTS.SPRITES);
      options.onClick = options.onClick || function (clicked) {};
      const columns = 3;
      const size = 62 + 50;
      options.w = columns * size;
      options.h = options.itemsPerPage / columns * size + size;
      const itemsPerPage = options.itemsPerPage;
      const spriteSize = options.spriteSize;
      const sprites = options.sprites;
      let currentPage = 0;
      const updatePage = () => {
        spriteContainer.removeChildren();
        const start = currentPage * itemsPerPage;
        const end = Math.min(start + itemsPerPage, sprites.length);
        for (let i = start; i < end; i++) {
          const sprite = new Sprite(sprites[i]);
          sprite.width = spriteSize;
          sprite.height = spriteSize;
          sprite.x = i % columns * (spriteSize + 10);
          sprite.y = Math.floor(i % itemsPerPage / columns) * (spriteSize + 10);
          sprite.enableEvents();
          sprite.on('pointerup', () => {
            options.onClick(sprites[i]);
          });
          spriteContainer.addChild(sprite);
        }
      };
      const spriteContainer = new UIContainer();
      spriteContainer.x = 10;
      spriteContainer.y = 10;
      pane.addChild(spriteContainer);
      const prevButton = game.render.component('hud_button', {
        name: '<',
        w: 50,
        h: 50
      ...
      nextButton.x = options.w - 60;
      nextButton.y = options.h - 60;
      nextButton.on('pointerup', () => {
        if ((currentPage + 1) * itemsPerPage < sprites.length) {
          currentPage++;
          updatePage();
        }
      });
      ...
      const drawBoxPane = game.render.component('drawBoxPane', drawBoxPaneOptions);
      drawBoxPane.addChild(pane);
      return drawBoxPane;
    },
    "parent": 'component_huds',
    "name": 'HUD Sprite Picker'
  };
  _BLUEPRINTS.COMPONENTS.hud_head_maker = {
    "getElement": function (options) {
      if (!options) {
        options = {};
      }
      const dummy = new LifeObject();
      ...
      drawer.x = options.w + 100;
      drawer.y = options.h / 2 - drawer.height / 2;
      const container = new UIContainer();
      container.addChild(drawer);
      const stage = game.render.componentRaw('squareWBorder', {
        w: 128,
        h: 128
      });
      ...
      stage.y = drawer.y;
      container.addChild(drawer);
      const layersPanel = new UIContainer();
      ...
      const updateLayersPanel = () => {
        ...
        layers.forEach((layer, index) => {
          const layerContainer = new UIContainer();
          layerContainer.y = index * 60;
          const layerName = game.render.text(layer.name, 'ingame-label');
          layerName.x = 10;
          layerName.y = 10;
          layerContainer.addChild(layerName);
          const removeButton = game.render.component('hud_button', {
            name: 'X',
            w: 30,
            h: 30
          });
          ...
          layerContainer.addChild(removeButton);
          const upButton = game.render.component('hud_button', {
            name: 'U',
            w: 30,
            h: 30
          });
          ...
          layerContainer.addChild(upButton);
          layersPanel.addChild(layerContainer);
        });
      };
      const updateLayerOrder = () => {
        drawer.removeChildren();
        layers.forEach(layer => {
          drawer.addChild(layer.sprite);
        });
      };
      const onClick = sprite => {
        const layer = game.render.componentRaw('hud_sprite_editor', {
          sprite
        });
        drawer.addChild(layer);
        layer.theSprite.scale.set(2);
        layers.push({
          name: sprite,
          sprite: layer
        });
      ...
      const tabComponents = [game.render.componentRaw('hud_sprite_picker', {
        onClick,
        sprites: heads
      }), game.render.componentRaw('hud_sprite_picker', {
        onClick,
        sprites: hats
      }), game.render.componentRaw('hud_sprite_picker', {
        onClick,
      ...
      const pane = game.render.component('component_container_tabs', {
        tabNames: tabNames,
        tabComponents: tabComponents,
        w: options.w,
        h: options.h
      ...
      pane.y = 70;
      container.addChild(pane);
      dummy.x = options.w / 2 - 70;
      dummy.y = options.h + 50;
      dummy.scale.set(1.5);
      dummy.setAnim("util/bandage", true);
      game.render.spine.hackSpineSlot("head_down", dummy.spine, game.render.draw.generateTexture(new Sprite('blank')));
      let counter = 0;
      let anim = "util/bandage";
      let ticks = 0;
      container.onTick(() => {
        counter++;
        ...
        if (ticks > 10) {
          ticks = 0;
          if (anim === "util/bandage") {
            anim = "none/walk_down";
            dummy.setAnim(anim, true);
          } else {
            anim = "util/bandage";
            dummy.setAnim(anim, true);
          }
        ...
        counter = 0;
        const clippingArea = new PIXI.Rectangle(drawer.width / 2 - 128, drawer.height / 2 - 128, 256, 256);
        const texture = game.render.draw.generateTexture(drawer);
        const spriteFromTexture = new PIXI.Sprite(texture);
        spriteFromTexture.scale.set(1);
        game.render.spine.hackSpineSlot("head_mask", dummy.spine, game.render.draw.generateTexture(spriteFromTexture, {
          region: clippingArea
        }));
        spriteFromTexture.destroy();
      });
      container.addChild(dummy);
      let currentBodyIndex = 0;
      const updateBody = () => {
        const body = bodies[currentBodyIndex];
        game.render.spine.hackSpineSlot("body_down", dummy.spine, game.render.draw.generateTexture(new Sprite(body)));
      };
      const leftButton = game.render.component('hud_button', {
        name: '<',
        w: 50,
      ...
      leftButton.x = options.w / 2 - 100;
      leftButton.y = options.h + 150;
      ...
      rightButton.x = options.w / 2 + 50;
      rightButton.y = options.h + 150;
      ...
      updateBody();
      const bannerNote = game.render.text("JamDonutGames.com - Survival and Simulation Games", 'ingame-label');
      bannerNote.x = 0;
      bannerNote.y = 0;
      container.addChild(bannerNote);
      const scrollNote = game.render.text("Scroll wheel rotates items", 'ingame-label');
      scrollNote.x = 0;
      scrollNote.y = 20;
      container.addChild(scrollNote);
      const rightClickNote = game.render.text("Right click deletes items", 'ingame-label');
      rightClickNote.x = 0;
      rightClickNote.y = 40;
      container.addChild(rightClickNote);
      const saveButton = game.render.component('hud_button', {
        name: 'Save',
        w: 100,
        h: 50
      });
      saveButton.x = options.w / 2 - 50;
      saveButton.y = options.h + 200;
      saveButton.on('pointerup', async () => {
        const renderTexture = PIXI.RenderTexture.create({
          width: container.width,
          height: container.height
        });
        game.render.draw.render(container, renderTexture);
        ...
        canvas.toBlob(async blob => {
         /* const item = new ClipboardItem({
          ...
          $.ajax(settings).done(function (response) {
            const imgur = JSON.parse(response);
            let link = imgur.data.link;
            link = link.replace(/.png/, "");
            link = link.replace(/i.img/, "img");
            setTimeout(() => {
  ...
  _BLUEPRINTS.COMPONENTS.hud_sprite_editor = {
    "getElement": function (options) {
      if (!options) {
        options = {};
      }
      options.sprite = options.sprite || "head_naked";
      const sprite = new Sprite(options.sprite);
      sprite.anchor.set(0.5);
      sprite.x = sprite.height / 2;
      sprite.y = sprite.width / 2;
      const container = new UIContainer();
      container.addChild(sprite);
      const increaseSizeButton = game.render.component('hud_button', {
        name: '+',
        w: 20,
        h: 20,
        fill: '0x3d3d3d'
      });
      increaseSizeButton.x = (container.width - 20) / 2;
      increaseSizeButton.y = 0;
      increaseSizeButton.visible = false;
      container.addChild(increaseSizeButton);
      increaseSizeButton.on('pointerup', () => {
        sprite.scale.x += 0.1;
        sprite.scale.y += 0.1;
      });
      const decreaseSizeButton = game.render.component('hud_button', {
        name: '-',
        w: 20,
        h: 20,
        fill: '0x3d3d3d'
      });
      decreaseSizeButton.x = (container.width - 20) / 2;
      decreaseSizeButton.y = container.height - 20;
      decreaseSizeButton.visible = false;
      container.addChild(decreaseSizeButton);
      decreaseSizeButton.on('pointerup', () => {
        sprite.scale.x -= 0.1;
        sprite.scale.y -= 0.1;
      });
      sprite.enableEvents();
      sprite.on('pointerup', () => {
        increaseSizeButton.visible = !increaseSizeButton.visible;
        decreaseSizeButton.visible = !decreaseSizeButton.visible;
      });
      ...
      container.on('pointerup', event => {
        if (event.target !== sprite && event.target !== rotateButton && event.target !== increaseSizeButton && event.target !== decreaseSizeButton) {
          increaseSizeButton.visible = false;
          decreaseSizeButton.visible = false;
        }
      });
      ...
      sprite.on('pointerdown', event => {
        ...
        dragOffsetX = localPosition.x;
        dragOffsetY = localPosition.y;
      });
      sprite.on('pointerup', () => {
        isDragging = false;
      });
      ...
      container.on('pointermove', event => {
        if (isDragging) {
          const localPosition = event.data.getLocalPosition(container.parent);
          container.x = localPosition.x - dragOffsetX;
          container.y = localPosition.y - dragOffsetY;
        }
      });
      container.enableEvents();
      container.on('pointerup', event => {
      ...
      sprite.hitArea = new PIXI.Rectangle(-sprite.width / 4, -sprite.height / 4, sprite.width / 2, sprite.height / 2);
      container.hitArea = new PIXI.Rectangle(-20, -20, container.width + 40, container.height + 40);
      container.on('rightdown', () => {
        container.destroy();
      });
      sprite.on('pointerover', () => {
        window.addEventListener('wheel', rotateSprite);
      });
      sprite.on('pointerout', () => {
        window.removeEventListener('wheel', rotateSprite);
      });
      function rotateSprite(event) {
        const delta = event.deltaY;
        sprite.rotation += delta > 0 ? 0.1 : -0.1;
      }
      container.theSprite = sprite;
      return container;
    },
    "parent": 'component_huds',
    "name": 'HUD Sprite Editor'
  };
